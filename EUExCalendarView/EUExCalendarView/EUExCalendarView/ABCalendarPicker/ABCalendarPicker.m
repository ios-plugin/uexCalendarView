                ...
                if (!needScrollPrev && !needScrollNext)
                {
                    if (!control.highlighted)
                    {
                        ...
                        [UIView animateWithDuration:0.3 animations:^{
                            [imageView setFrame:CGRectMake(control.frame.origin.x+8, control.frame.origin.y+control.frame.size.width*2, control.frame.size.width-16, 8)];
...
- (void)updateColumnNamesForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider
{
    ...
    for (int j = 0; j < columnsCount; j++)
    {
...
#endif
        columnLabel.backgroundColor = [UIColor whiteColor];
		...
		columnLabel.font = [UIFont boldSystemFontOfSize:12.0f];
...
//    self.titleLabel.textColor = self.styleProvider.textColor;
    self.titleLabel.textColor = [EUtility ColorFromString:@"#0073be"];
...
- (void)updateButtonsForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider andState:(ABCalendarPickerState)state
{
    ...
    for (int i = 0; i < rowsCount; i++)
    {
        ...
        for (int j = 0; j < columnsCount; j++)
        {
            ...
            UIImageView *animateView  = (UIImageView *)[self viewWithTag:100];
            if (controlState == 5) {
                if (!animateView) {
...
//                    [animateView setBackgroundColor:[UIColor redColor]];
                    ...
                    [UIView animateWithDuration:0.3 animations:^{
                        [animateView setFrame:CGRectMake(j*buttonWidth-1+8,  (i+1)*buttonHeight+buttonHeight, buttonWidth+1+shift-16, 8)];
                    } completion:^(BOOL finished) {
    ...
    [self.nowTileView setBackgroundColor:[UIColor whiteColor]];
...
//        self.mainTileView.backgroundColor = [UIColor colorWithRed:164/255. green:167/255. blue:176/255. alpha:1.0];
        self.mainTileView.backgroundColor = [UIColor clearColor];
